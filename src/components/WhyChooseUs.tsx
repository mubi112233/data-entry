...
export const WhyChooseUs = () => {
  ...
  return (
    <motion.section 
      ref={ref}
      className="relative py-8 sm:py-10 md:py-12 lg:py-14 overflow-hidden z-40 bg-gradient-to-br from-green-900 via-green-900 to-green-950"
      initial={{ opacity: 0 }}
    ...
    >
      ...
      <motion.div
        className="absolute top-0 right-0 w-[500px] h-[500px] bg-green-700/20 rounded-full blur-[120px] z-0"
        style={{ y: springY }}
      ...
      />
      <motion.div
        className="absolute bottom-0 left-0 w-[400px] h-[400px] bg-green-600/15 rounded-full blur-[100px] z-0"
        style={{ y: springY }}
      ...
      />
      <motion.div
        className="absolute top-1/2 left-1/3 w-[300px] h-[300px] bg-green-500/10 rounded-full blur-[80px] z-0"
      ...
      <div className={`container mx-auto ${SPACING.container} relative z-10`}>
        ...
        >
          ...
          >
            <Sparkles className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-green-300" />
            <span>{badge}</span>
          </motion.div>
          <h2 
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5 sm:gap-6 md:gap-8 max-w-7xl mx-auto">
          {items.map((item, index) => {
            ...
            <motion.div 
              key={index}
              className="relative bg-white/5 border border-white/10 rounded-xl sm:rounded-2xl p-6 sm:p-8 md:p-10 hover:border-green-500/50 hover:shadow-[0_25px_80px_-20px_rgba(34,197,94,0.4)] transition-all duration-700 group overflow-hidden"
            ...
            >
              ...
              <div className="relative z-10">
                <motion.div 
                  className="mb-4 sm:mb-5 md:mb-6 inline-flex p-3 sm:p-4 md:p-5 rounded-lg sm:rounded-xl bg-green-500/20 text-green-300 group-hover:bg-green-600 group-hover:text-white group-hover:scale-110 transition-all duration-500 shadow-[0_10px_30px_-10px_rgba(34,197,94,0.4)]"
                  whileHover={{ rotate: [0, -10, 10, -10, 0] }}
                  transition={{ duration: 0.5 }}
                >
                  <IconComponent className="w-8 h-8 sm:w-9 sm:h-9 md:w-10 md:h-10" />
                </motion.div>
                <h3 className="text-xl sm:text-2xl font-bold mb-3 sm:mb-4 text-white group-hover:text-green-300 transition-colors duration-300">
                  {item.title}
                </h3>
